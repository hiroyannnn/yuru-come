...
- **左「気になるコメント」**: 拾うならこのあたり、という候補です。種類のバッジ、拾う度（priority）、投稿者とプラットフォーム（Twitch はチャンネル名も）。クリックで既読にして消します。5 分たつと薄くなり、10 分で消えます。配信トラブルの報告は赤い枠で最上位に固定します。並びは priority の高い順、同じなら新しい順です。同じコメントが重ねて来たら「同じコメント ×3」と出ます。
