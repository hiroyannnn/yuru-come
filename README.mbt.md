...
| 拾うべきか: 種類 または priority ≥ 1.5（既定） | 適合率 97.2% / 再現率 97.2% | 適合率 62.1% / 再現率 100% |
...
本家 Jev（`--workers 4`）で別の配信中チャンネル（約 1 件/秒、4 分で 155 人が発言）を流すと、246 件のうち 171 件を Jev に投げて失敗も間引きも 0 件、拾い上げはコメントの到着から 0.3〜0.6 秒で出ました。ただし、実際のチャットでは視聴者どうしの掛け合いが question / request / correction と判定されることが多く、4 分で 54 件が拾い上げに入りました。その 6 割は priority が 0.5 未満（Jev は「拾わなくてよい」と見ている）です。評価セットのような「配信者に向けたコメント」だけなら種類で拾って問題ありませんが、実配信では種類に加えて priority の下限も見たほうがよさそうです（未実装）。
...
- 拾うべき種類（question など）は priority が低くても拾います。本家 Jev では、視聴者どうしの掛け合いが拾い上げに入りすぎます（上の「流速に追いつけるか」を参照）。
