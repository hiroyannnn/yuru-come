...
| `--pickup-floor <0..2>` | `0.6` | 質問・リクエスト・指摘・初見を拾うのに求める priority の下限。0 にすると種類だけで拾う。trouble は常に拾う |
...
| `trouble` | 配信トラブルの報告（音が出ていない、画面が止まった等） | 必ず拾う（最上位に固定） |
...
質問やリクエストにも下限（`--pickup-floor`、既定 0.6）があるのは、実際のチャットでは視聴者どうしの掛け合い（「〜でいいでしょw」「〜じゃね？」）が request や question と判定されるからです。Jev はそういうコメントに 0.5 未満の priority を付けるので、下限で落とせます。評価セットの「拾うべき」コメントは、本家 Jev で最小でも 0.67 でした。
...
| 既定（種類は priority ≥ 0.6） | 136 件 | 10 件 |

種類だけで拾うと、視聴者どうしの掛け合いが question / request / correction と判定されて 4 分で 54 件も入り、その 6 割は priority が 0.5 未満でした。下限を入れると、question / request / correction と判定された 11 件のうち拾うのは 4 件になりました。残る誤りは trouble です。trouble は必ず拾うので、ゲーム内の出来事を嘆くコメント（priority 0.4〜0.8）が trouble と判定されると最上位に出ます（10 件のうち 5 件が trouble で、本当に配信の不具合だったのは 2 件）。
...
- trouble は priority が低くても必ず拾って最上位に固定します。実際のチャットでは、ゲーム内の出来事を嘆くコメントが trouble と誤判定されて上に出ることがあります（上の「流速に追いつけるか」を参照）。
- `--twitch` に指定できるチャンネルは 1 つです。
